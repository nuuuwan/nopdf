...
REGEX_DATE = r'^(?P<date>\d{2}\s*\.\d{2}\.\d{4})'
REGEX_DATE2 = r'^(?P<date>\d{4}\.\d{2}\.\d{2})'
...
REGEX_PLACE_DEATHS = (
    r'(?P<place>.*(Residence|hospital).*)\s*-' + r'\s*(?P<deaths>\d+).*'
)
REGEX_GENDER_AGE_DEATHS = (
    r'^(?P<age>.+).+(?P<male>(Ol|\dA,|-|\d{2})).+'
    + r'(?P<female>(Ol|\dA,|-|\d{2})).+(?P<total>(Ol|\dA,|-|\d{2}))'
)
